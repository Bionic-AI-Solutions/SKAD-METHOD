...
edge_case_hunter_task: '{project-root}/_skad/core/tasks/review-edge-case-hunter.xml'
...
**One-shot:** Skip diff construction. Still invoke the `skad-review-adversarial-general` skill in a subagent with the changed files — inline review invites anchoring bias.
...
- **Blind hunter** — receives `{diff_output}` only. No spec, no context docs, no project access. Invoke via the `skad-review-adversarial-general` skill.
