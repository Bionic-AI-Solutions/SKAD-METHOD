...
Prepare and optionally launch Ralph Wiggum autonomous loop execution for a SKAD story.
...
2. Check for `ralph-skad.sh` at the project root.
   - If missing, generate the SKAD-aware wrapper script (updates story status ready-for-dev → in-progress → review, wraps ralph.sh).
3. Make both scripts executable: `chmod +x ralph.sh ralph-skad.sh`
...
- [ ] ralph-skad.sh exists and is executable
...
- **[L] Launch now**: Execute `./ralph-skad.sh {{story_file}} {{max_iterations}}`
...
./ralph-skad.sh {{story_file}} {{max_iterations}}
