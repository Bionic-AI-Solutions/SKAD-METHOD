...
Audit new-format SKAD source files for file-reference convention violations using parallel Haiku subagents.

## Convention

In new-format SKAD workflow and task files (`src/bmm/`, `src/core/`, `src/utility/`), every file path reference must use one of these **valid** forms:

- `{project-root}/_skad/path/to/file.ext` — canonical form, always correct
...
- `steps/step-NN.md` — bare steps-relative path (missing `{project-root}/_skad/...` prefix)
- `` `_skad/core/tasks/help.md` `` — bare `_skad/` path (missing `{project-root}/`)
...
3. For each batch, spawn a subagent (`subagent_type: "Explore"`, `model: "haiku"`) with this prompt (fill in the actual file paths):
   ...
   >
   > **Valid references:** `{project-root}/_skad/...`, `{installed_path}/...`, template variables.
   > **Flag:** bare filenames (`step-NN.md`), `./` or `../` relative paths, bare `steps/` paths, bare `_skad/` paths (without `{project-root}/`), absolute system paths.
   >
