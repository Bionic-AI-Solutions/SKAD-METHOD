...
module.exports = {
  command: 'install',
  description: 'Install SKAD Core agents and tools',
  options: [
    ['-d, --debug', 'Enable debug output for manifest generation'],
    ['--directory <path>', 'Installation directory (default: current directory)'],
    ['--modules <modules>', 'Comma-separated list of module IDs to install (e.g., "skm,bmb")'],
    [
      '--tools <tools>',
    ...
    ['--output-folder <path>', 'Output folder path relative to project root (default: _skad-output)'],
  ...
  action: async (options) => {
    try {
      ...
      if (options.debug) {
        process.env.SKAD_DEBUG_MANIFEST = 'true';
