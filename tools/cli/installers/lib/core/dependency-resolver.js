...
class DependencyResolver {
  ...
  parseCommandReferences(content) {
    ...
    const pathPattern = /skad\/(core|bmm|cis)\/(agents|tasks)\/([a-z0-9-]+)/g;
  ...
  async resolveSingleDependency(skadDir, dep) {
    ...
    switch (dep.type) {
      ...
      case 'skad-path': {
        ...
        // skad/bmm/tasks/bar.md -> src/bmm/tasks/bar.md (bmm is directly under src/)
    ...
    for (const file of files) {
      ...
      if (file.includes('/src/core/') || file.includes('/src/bmm/')) {
        if (module === 'core') {
          ...
          moduleBase = path.join(skadDir, 'src', 'bmm');
        }
      } else {
