...
/**
 * Helpers for gathering SKAD agents/tasks from the installed tree.
 * Shared by installers that need Claude-style exports.
 */
async function getAgentsFromSkad(skadDir, selectedModules = []) {
  const agents = [];

  // Get core agents
  if (await fs.pathExists(path.join(skadDir, 'core', 'agents'))) {
    const coreAgents = await getAgentsFromDir(path.join(skadDir, 'core', 'agents'), 'core');
    agents.push(...coreAgents);
  }

  // Get module agents
  for (const moduleName of selectedModules) {
    const agentsPath = path.join(skadDir, moduleName, 'agents');
...
async function getTasksFromSkad(skadDir, selectedModules = []) {
  const tasks = [];

  if (await fs.pathExists(path.join(skadDir, 'core', 'tasks'))) {
    const coreTasks = await getTasksFromDir(path.join(skadDir, 'core', 'tasks'), 'core');
    tasks.push(...coreTasks);
  }

  for (const moduleName of selectedModules) {
    const tasksPath = path.join(skadDir, moduleName, 'tasks');
