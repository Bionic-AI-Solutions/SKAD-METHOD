...
class WorkflowCommandGenerator {
  ...
  async generateCommandContent(workflow, skadDir) {
    ...
    // To: {project-root}/_skad/bmm/workflows/.../workflow.yaml
    ...
    if (workflowPath.includes('/src/bmm/')) {
      ...
      if (match) {
        workflowPath = `${this.skadFolderName}/bmm/${match[1]}`;
      }
...
`;
  ...
  transformWorkflowPath(workflowPath) {
    ...
    if (workflowPath.includes('/src/bmm/')) {
      const match = workflowPath.match(/\/src\/bmm\/(.+)/);
      if (match) {
        transformed = `{project-root}/${this.skadFolderName}/bmm/${match[1]}`;
      }
    } else if (workflowPath.includes('/src/core/')) {
      const match = workflowPath.match(/\/src\/core\/(.+)/);
      if (match) {
   ...
   * Creates flat files like: skad_bmm_correct-course.md
   *
  ...
  async writeColonArtifacts(baseCommandsDir, artifacts) {
    ...
    for (const artifact of artifacts) {
      if (artifact.type === 'workflow-command') {
        // Convert relativePath to underscore format: bmm/workflows/correct-course.md → skad_bmm_correct-course.md
   ...
   * Creates flat files like: skad-bmm-correct-course.md
   *
  ...
  async writeDashArtifacts(baseCommandsDir, artifacts) {
    ...
    for (const artifact of artifacts) {
      if (artifact.type === 'workflow-command') {
        // Convert relativePath to dash format: bmm/workflows/correct-course.md → skad-bmm-correct-course.md
