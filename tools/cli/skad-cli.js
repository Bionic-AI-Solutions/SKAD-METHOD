  ...
  try {
    ...
    if (result && result !== packageJson.version) {
      ...
      const updateMsg = [
        ...
        `  npm cache clean --force && npx skad-method@${tag} install`,
      ].join('\n');
...
// Set up main program
program.version(packageJson.version).description('SKAD Core CLI - Universal AI agent framework');
