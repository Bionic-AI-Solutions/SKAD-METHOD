...
Create a reference documentation page at `docs/reference/modules.md` that lists all official external SKAD modules with descriptions and links.
...
- Start with a brief intro explaining that SKAD extends through official modules selected during installation
...
- Include a `:::tip` admonition about how to install modules (via `npx skad-method` installer)
- Mention that community modules and a marketplace are coming
- Do NOT include built-in modules (core, skm) — this page is specifically for external/add-on modules
