...
SKAD provides two testing paths: a built-in QA agent for fast test generation and an installable Test Architect module for enterprise-grade test strategy.
...
| **Setup** | Nothing to install -- included in SKM | Install separately via `npx skad-method install` |
...
Quinn is the built-in QA agent in the SKM (Agile suite) module. It generates working tests quickly using your project's existing test framework -- no configuration or additional installation required.

**Trigger:** `QA` or `skad-skm-qa-automate`
...
Quinn's Automate workflow appears in Phase 4 (Implementation) of the SKAD Method workflow map. A typical sequence:
