...
# Ralph Wiggum - SKAD Autonomous Pipeline
# ========================================
# Autonomous story-chaining pipeline that discovers, executes, validates,
# reviews, and chains SKAD stories without human intervention.
#
# Phase Flow:
...
#
# Usage: ./ralph-skad.sh [story_file] [max_retries_per_task] [max_cr_iterations]
#        ./ralph-skad.sh                    # Auto-detect + chain mode
#        ./ralph-skad.sh story.md 3 3       # Single story (explicit)
#
...
find_sprint_status() {
  ...
  echo "${path:-./_skad-output/implementation-artifacts/sprint-status.yaml}"
}
...
check_epic_complete() {
  ...
  local epic_validation="_skad-output/implementation-artifacts/epic-${epic_num}-validation.sh"
...
trigger_create_story() {
  ...
  local expected_file="_skad-output/implementation-artifacts/${cs_key}.md"
...
echo ""
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}   Ralph-SKAD Autonomous Pipeline${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""
